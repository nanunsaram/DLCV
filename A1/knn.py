...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        train_vec = x_train[i].view(-1)
        for j in range(num_test):
            test_vec = x_test[j].view(-1)
            squared_diff = (train_vec - test_vec)**2
            dists[i][j] = torch.sum(squared_diff)**(1/2)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        train_vec = x_train[i].view(-1)
        test_vec_mat = x_test.view(num_test, -1)
        squared_diff_mat = (test_vec_mat - train_vec)**2
        dists[i] = torch.sum(squared_diff_mat, dim=1)**(1/2)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    train_vec_mat = x_train.view(num_train, -1)
    test_vec_mat = x_test.view(num_test, -1)

    train_sq = torch.sum(train_vec_mat**2, dim=1, keepdim=True)
    test_sq = torch.sum(test_vec_mat**2, dim=1, keepdim=True)
    cross_terms = torch.mm(train_vec_mat, test_vec_mat.t())

    dists = (train_sq + test_sq.t() - 2*cross_terms)**(1/2)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    # Replace "pass" statement with your code
    for j in range(num_test):
        y_pred[j] = torch.bincount(y_train[torch.topk(dists[:, j], k=k, dim=0, largest=False).indices]).argmax()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    for k in k_choices:
        ...
        for j in range(num_folds):
            classifier = KnnClassifier(torch.cat
                        (x_train_folds[:j]+x_train_folds[j+1:]),
                        torch.cat(y_train_folds[:j]+y_train_folds[j+1:]))
            accuracy = classifier.check_accuracy(x_train_folds[j],
                                      y_train_folds[j], k=k, quiet=True)
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k, accs in sorted(k_to_accuracies.items()):
        if statistics.mean(accs) > best_accs:
            best_accs = statistics.mean(accs)
            best_k = k
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
