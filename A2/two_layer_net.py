...
def nn_forward_backward(
    ...
    probs = torch.softmax(scores, dim=1) #(N, C)
    probs_correct_class = probs[torch.arange(N), y] #(N,)
    loss = -torch.sum(torch.log(probs_correct_class))/N + reg*(torch.sum(W1*W1) + torch.sum(W2*W2))
    ...
    grad_scores[torch.arange(N), y] -= 1.0
    grad_scores /= N

    grads['W2'] = torch.mm(h1.t(), grad_scores) + 2*reg*W2
    grads['b2'] = grad_scores.sum(dim=0)

    grad_h = torch.mm(grad_scores, W2.t())
    grad_h[h1 == 0] = 0 #ReLU

    grads['W1'] = torch.mm(X.t(), grad_h) + 2*reg*W1
    grads['b1'] = grad_h.sum(dim=0)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_get_search_params():
    ...
    learning_rate_decays = [0.95, 0.90, 0.80]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
      ...
      for hs in hidden_sizes:
        for reg in regularization_strengths:
          for lr_decay in learning_rate_decays:
            cur_net = TwoLayerNet(3 * 32 * 32, hs, 10, device=data_dict['X_train'].device,
            dtype=data_dict['X_train'].dtype)
            cur_stat = cur_net.train(data_dict['X_train'], data_dict['y_train'], data_dict['X_val'], data_dict['y_val'],
            learning_rate=lr, learning_rate_decay=lr_decay,
            reg=reg, num_iters=3000, batch_size=1000,verbose=False)
            
            y_val_pred = cur_net.predict(data_dict['X_val'])
            cur_val_acc = 100.0 * (y_val_pred == data_dict['y_val']).double().mean().item()
    
            if best_net == None:
              best_net = cur_net
              best_stat = cur_stat
              best_val_acc = cur_val_acc
            elif cur_val_acc > best_val_acc:
              best_net = cur_net
              best_stat = cur_stat
              best_val_acc = cur_val_acc              
