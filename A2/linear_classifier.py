...
def svm_loss_vectorized(
    ...
    #############################################################################
    # Replace "pass" statement with your code
    score_vec = torch.mm(X, W)  # score_vec has shape (N, C)
    correct_class_score_vec = score_vec[torch.arange(X.shape[0]), y].view(-1, 1)
    margin = torch.maximum(torch.zeros_like(score_vec),
                            score_vec - correct_class_score_vec + 1)
    loss = torch.sum(margin) / (X.shape[0]) - 1 + reg * torch.sum(W * W)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    ...
    #############################################################################
    # Replace "pass" statement with your code
    margin_mask = (margin > 0).int()
    margin_mask[torch.arange(X.shape[0]), y] -= torch.sum(margin_mask, dim=1)
    dW += torch.mm(X.t(), margin_mask.to(W)) / X.shape[0] + 2 * reg * W
    # X.t() is (D, N), margin_mask is (N, C)
    # margin_mask[i, j] makes addition/subtraction of X[i] to
    # the `j`th column of the dW
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
...
def svm_get_search_params():
    ...
    regularization_strengths = [5e-2, 1e-1, 5e-1, 1e0, 2e0]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def test_one_param_set(
    ...
    # Replace "pass" statement with your code
    cls.train(X_train = data_dict['X_train'], y_train = data_dict['y_train'],
    ...
    val_acc = 100.0 * (data_dict['y_val'] == y_val_pred).double().mean()
    ############################################################################
    #                            END OF YOUR CODE                              #
    ############################################################################
